...
class RsaFragment: BaseFragment<FragmentRsaBinding>() {
    ...
    override fun onViewInit() {
        ...
        decryptCode(encryptCode("test123", publicKey = publicKey),privateKey)
    }
    ...
    private fun encryptCode(code: String,publicKey: RSAPublicKey): ByteArray? {
        //指定要使用RSA/ECB/PKCS1Padding加密解密方式
        val cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding")
        cipher.init(Cipher.ENCRYPT_MODE, publicKey)     //初始化加密
        val encrypted = cipher.doFinal(code.toByteArray(Charsets.UTF_8)) //使用RSA公鑰加密後得到結果
        Log.d("***********", "加密後: ${encrypted}")
        return encrypted
    }

    private fun decryptCode(code: ByteArray?,privateKey: RSAPrivateKey): String {
        val cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding")
        cipher.init(Cipher.DECRYPT_MODE, privateKey)  //初始化解密
        val decrypted = cipher.doFinal(code)     //使用私鑰解密
        val decryptedText = String(decrypted, Charsets.UTF_8) //獲得解密
        Log.d("***********", "解密後: ${decryptedText}")
